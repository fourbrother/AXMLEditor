...
public class XmlEditor {
	 ...
	 * @param tagName
	 * @param attrName
	 * @return int 返回删除属性的偏移地址，-1表示没找到要删除的属性
	 */
	public static int removeAttr(String tag, String tagName, String attrName) {
		int attrIndex = -1;
		try{
			ParserChunkUtils.parserXml();
				...
				if(tag.equals(tagNameTmp)){
					...
					if(tag.equals("application") || tag.equals("manifest")) {
						for(AttributeData data : chunk.attrList) {
							attrIndex++;
							String attrNameTemp1 = ParserChunkUtils.xmlStruct.stringChunk.stringContentList.get(data.name); // 标签名的key  data.name
						...
						// application manifest tag里没有找到属性 值为 -1
						return attrIndex;
					}

					//否则需要通过name找到 四大组件 指定的tag
					String nameAttrValue = "";
					for(AttributeData attrData : chunk.attrList){
						String attrNameTemp = ParserChunkUtils.xmlStruct.stringChunk.stringContentList.get(attrData.name); // 属性名的key  attrData.name
						if("name".equals(attrNameTemp)){//得先找到tag对应的唯一名称
							nameAttrValue = ParserChunkUtils.xmlStruct.stringChunk.stringContentList.get(attrData.valueString); // 获取name的值
						}
					}

					// 前面没找到，重新找
					if(tagName.equals(nameAttrValue)){ // 找到tag唯一标识后，才进行删除操作
						for(AttributeData data : chunk.attrList){
							attrIndex++;
							String attrNameTemp1 = ParserChunkUtils.xmlStruct.stringChunk.stringContentList.get(data.name); // 属性名的key  data.name
							if(attrName.equals(attrNameTemp1)){ // 删除需要找到有这个属性
								//如果找到对应的标签，发现只有一个属性值，并且删除成功，同时还得把这个标签给删除了
								if(chunk.attrList.size() == 1){
									removeTag(tag, tagName);
									return -1;
	...
	public static void modifyAttr(String tag, String tagName, String attrName, String attrValue){
		try{
			ParserChunkUtils.parserXml();
			int attrIndex = XmlEditor.removeAttr(tag, tagName, attrName);
			ParserChunkUtils.parserXml();
			XmlEditor.addAttr(tag, tagName, attrName, attrValue, attrIndex);
		}catch (IOException e){
			System.out.println("parse xml err:"+e.toString());
		}
	}
	 ...
	 * @param attrValue
	 */
	public static void addAttr(String tag, String tagName, String attrName, String attrValue, int attrIndex){
		try{
			ParserChunkUtils.parserXml();
			//构造一个属性出来
			...
			for(StartTagChunk chunk : ParserChunkUtils.xmlStruct.startTagChunkList){
				...
				if(tag.equals(tagNameTmp)){
					//如果是application，manifest标签直接处理就好
					if(tag.equals("application") || tag.equals("manifest")){
						// 还得修改对应的tag chunk中属性个个数 和 大小
						// 修改chunk的属性个数
						int countStart = chunk.offset + 28; // 设置属性个数的偏移地址
						byte[] modifyByte = Utils.int2Byte(chunk.attrList.size()+1); // 这里是要修改的值
						ParserChunkUtils.xmlStruct.byteSrc = Utils.replaceBytes(ParserChunkUtils.xmlStruct.byteSrc, modifyByte, countStart); // 替换修改

						// 修改chunk的大小
						int chunkSizeStart = chunk.offset + 4; // 大小的偏移地址
						int chunkSize = Utils.byte2int(chunk.size); // chunk的当前大小
						int chunkInsertOffset = chunkSize;
						if (attrIndex != -1) {
							chunkInsertOffset = 36 + attrIndex * 20; // 插入属性的偏移地址 attrIndex 表示从0开始的下标
						}
						byte[] modifyByteSize = Utils.int2Byte(chunkSize + 20);
						ParserChunkUtils.xmlStruct.byteSrc = Utils.replaceBytes(ParserChunkUtils.xmlStruct.byteSrc, modifyByteSize, chunkSizeStart); // 替换修改

						// 添加属性内容到原来的chunk上（直接加到末尾了，xml解析数组，偏移地址，属性的byte形式的内容）
						// 在这就是直接加到末尾了
						System.out.println("chunkSize: " + chunkSize);
						System.out.println("attr chunk insert offset: " + chunkInsertOffset);
						ParserChunkUtils.xmlStruct.byteSrc = Utils.insertByte(ParserChunkUtils.xmlStruct.byteSrc, chunk.offset + chunkInsertOffset, data.getByte());
					...
					if(tagName.equals(nameAttrValue)) { // 找到tag唯一标识后，直接添加属性。不用找到需要添加的属性，默认没有
						// 还得修改对应的tag chunk中属性个个数和大小
						int countStart = chunk.offset + 28;
						byte[] modifyByte = Utils.int2Byte(chunk.attrList.size()+1);
						ParserChunkUtils.xmlStruct.byteSrc = Utils.replaceBytes(ParserChunkUtils.xmlStruct.byteSrc, modifyByte, countStart);
						...
						//修改chunk的大小
						int chunkSizeStart = chunk.offset + 4; // 大小的偏移地址
						int chunkSize = Utils.byte2int(chunk.size); // 整个tag的长度
						int chunkInsertOffset = chunkSize;
						if (attrIndex != -1) {
							chunkInsertOffset = 36 + attrIndex * 20; // 插入属性的偏移地址 attrIndex 表示从0开始的下标
						}
